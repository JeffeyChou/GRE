...
> CEH -> BEH
> 这里第一空出错，主要就是没有注意语义。后面提到了应该注意 `interaction` 而不是 `individual factors` ，所以前面提到 `each of them` 是不能达到这样的效果，所以选择B。
...
> CF -> CE
> 前面的虚假得到了，但是第二空没有转过来。既然前面说明是虚假，那么第二空就要和 `lurid thrillers` 取反义，也就是正面的词汇，所以选择E。
> radical political agenda的相对应（同义） high-minded purpose。
...
> BDH -> BFH
> 从3空入手，even if 资金来源不重要，但是资金来源（iii)了信任。所以填vitiate。credibility 对应skepticism，因为A、C两空的核心意思完全无关所以排除。第一句和第二句中间没有任何转折的词汇，没有but，however，也没有disagree，所以两句话的意思是一致的（对我知道很同意看到后边的fund选错）
> 第二空没有遵循情感一致，这里应该也是取负面词，所以是F。
...
> CD -> AD
> 这里就没有遵循语义一致和逻辑一致了，第一空前面提到 `candidate wins` 那么第一空就取能win的A，第二空 看到 `without their help` ，就取能失败的D。
...
E. triviality
>  A -> A
>  这里没有找到明显的对比主体词，我找到的关键词是 `firendliness`, `stifled`。其实真正的主体词是 `a small gesture` ，所以后面 `not without __ = not small` 选择A
...
> C -> B
> 这里同上，没有找到明显的主体词。只能根据后面的`destructive and dangerous` 推测前面应该需要避免，选择了C。
...
> 这里第一空看到 and ，以为是并列，所以选择和 `merits`同义的A。
> 第二个就不懂了，在DF间最后选择了F
> 首先观察仔细的话，你发现第一个and并不是并列关系，而是另外起一个，应该是对比关系， `considering ways to __ them` 中的 `them` 就是前面的的第一空。加上后面的 `deficiencies` 就是重复第一空，`address` 就是重复第二空，那么可以选出AD。
...
> BD -> AD
> 首先既然你确定了D，那么根据意思你不应该选到B。这里![[populist]]
> 第二空和第一空联系起来，所以是A
> 
...
> BEG -> CEG
> 当时自己填的就觉得B怪怪的。。你要记住ETS不说废话，很多东西都有暗示甚至重复。这里可以看到第一和第二空是取反意思，可以得到CE答案。`connections` 对应 `contacts` ，`limited` 对应 `extensive`。
...
> D -> E
> `parallels between human communities and insect colonies` 指的是相似之处，相似之处怎么会有 `contradictory` 这种修饰？填一个相似的其它词，即A。
> 即都有与同类的合作，只不过对这种合作的解读比较肤浅，没有深入探究是否和亲属合作。因而superficial是更贴切的。即便是这两种合作模式不一样，也没到有矛盾的程度，所以不选contradictory。
> 
...
> BF -> AF
> 第一空中，哪怕不看题目，跟report相关的好像只要A最合适
> 其次，`rather than` 构成一个 对比，第一空和第二空取反，所以选择 AF
> 先做第二空，rather than引导的取反，相较于对subject做彻底的研究，这个report倾向于开发新的研究线。对于一个现存对象的彻底研究与放弃彻底研究，转而研究新的对象之间有一个对立，所以第二空选thorough。冒号取同，句子里冒号后面的内容都是用来解释第一空，因为这个报告倾向于开启新的研究线而非彻底研究现有主题，因为这份报告是彻底的（thorough）反面，即粗略的，不完备的（sketchy）。flippant是指轻率的，与thorough不构成绝对反义。而且flippant如果形容一个决定很轻率还比较好理解，形容一份报告很轻率并不贴切。
...
> AEH -> AEG 
> 首先这里 `taciturn` 和 `reticent` 构成同义词，放在第三空明显不合适，其次这里有重复前面的 `fastidious of her apprearance` ,类似于 第二空对应 `accuse her of being either taciturn` 的关系，所以这里选择G而不是H。
...
> 一空，根据understanding of the difficulty of doing direct research,说明“理解了他做直接研究的难度”，所以这种批评被减少了。secondary sources = previous histories 所以在他的研究中playing a large role。surprising看出前半句和后半句取反，后半句说So没采访没direct research，前半应该是表示“So本来可以direct research”，第三空只有proximity to 符合意思。
...
> recent和19世纪的对比，肯定是正向的，更宏大的（large-scale），但是只是proposal，不一定effective；
> 二三空in contrast取反，第二空productive=devastating取反，第三空impotence是ambition取反。
...
> BFI -> CDH
> neat categories将女性标签化、符号化，but转折，女性反抗这样的分类；female loners独来独往者，与uniformly sociable pioneers好交际的先驱 取反，这样的对欧美女性一概而论是轻率肤浅的。
> 二空：loners(example) 和 sociable(generalizations)构成取反，➡️puncture削弱  > 三空：负态度➡️facile浅显的
...
> BE -> DF
> 第一句说对整体评价不高，那取反一定就是对个别评价高，也就是第二句的意思。fail再来个取反，空格填负向词
> 我们都知道although前后两个句子的关系是对比关系，那么前面有一个“do not”, 后面又一个“fail”,其实就跟做简单的数学题一样，等式的左右都有一个负号，我们就同时约掉。现在前面就剩“think highly of”, 前后句由于存在一个although，于是取反，后面应该是表示跟“称赞”相反的词汇，那么，DF正好符合逻辑

**发现带有although 之类的转折都可以用以下公式： although = not ，需要配平两边正负号即可。比如这里 although + not  消去，左边剩下 think highly of, 右边剩下 fail with __ , 那么not + 空 = think highly of， 所以空里面填一个negative 的词汇，也就是DF**
...
> BDH -> BDI
> 首先第一空对应problem, 填入负面词，最合适就是B。第二空和`unaware of limitations` 对应，填入B。第三空则是根据第二空的 `belie` 填入和 `relible` 相反的词，这里最好的就是 I。
...
> AD -> BD
> 一开始纠结A还是B。然后我想到 `decline` 是一个负面，前面对比的话需要一个positive的词汇，这里就选择了更正面的A而不是B。
> definitive; authoritative and apparently exhaustive ; sobering: : making you feel serious and thoughtful 这里结合题意的话是说这个数据使其feel serious and thoughtful.
...
> BD -> BF
> 一个负面的影响反之可以驱动一个满意的员工quit。otherwise提示了要选正面的词。而且 untested 意思也不对上。![[untested]]
...
> 这里首先提到 Because了，那么后面应该出现和 because 重复`personal favorite`的语义，这里即 `not __ = personal favorite` ，所以不应该是D，而应该是F
...
> AE -> BD
> 这里没有看懂后面最后一个冒号的意思是说这里其实是充满了 `personality`的，所以和前面的说法矛盾，所以选择BD。
...
> E ->D
> 这里自己看错了，以为填空里的和 `possessing a fluid energy` 对应，就选到了E。其实这是取反后的结果，注意到 `deliberately` `painter attempts to control` 可以看出D是更合适的。
...
> CD -> CE
> 这里第二空和第一空联动，![[pedestrian]] 取其commonplace和 everyday presence同义。
...
> AC -> DE
> 主要是读不懂，后面dispelled that presumption 表示的就是 前面的 were thought to 这个假设， 后面说的是 证明有关系 那排除的就是前面 没关系的假设 所以空填负向
...
> DF -> AB
> 被这生词绕晕了，然后自己也没有记清楚A和B其实就是同义词，当时只觉得B合适，凑不出同义词，就选到了DF

**6选2以及机经词已经出现了基本全部的选项词，也就是说你的正确答案应该和这些有关，不要乱选自己不认识的单词**
...
> AF -> AE
> 这个题更强调没有其他的产品与这个公司竞争，所以insulate更加合适
> help insulate the company （from 。。。） 某个原因让这个公司的发展从这个不景气的经济情况重隔离出来了，意思就是让他不像其他企业那样倒闭，就他一个人flourish。
...
> 其实从后面反推过来可可以得到第三空的答案： yet 后面和他具体的贡献对比得到他是得到不对称的评价，所以选择I。那么反推第二空是第三空的反义，所以推出D.

**句子结尾中有个if __ 属于让步成分，从当转折作用，做题时取 __ 的反义。** (参考section 81 T7)
...
> the belief不利于学生立场，所以根据前后整体语义一致的联动来判断，impasse对belief发出正面动作，Students会给予负面评价；根据选项，只有AE符合。
> 本来administration就是相信学生不应该插手，现在陷入僵局了，他们更不爽了，属于火上浇油了就是说，于是加强了原有的belief，这对学生们来说当然是不幸的，所以选A。
...
> 首先推出第三空，”we have volunteer home“，那就是说我们是自己建房子而不是靠政府（即纳税人），所以是absence. 整段话前后两个观点是相反的，第一句是政府要给居民建房子，第二句是人民自己建（即政府不建），由此推出对第一个空应该是说之前政府建造的观点已经（被代替/过时）（反正是要填一个负面的词），然后第二个空也就对应出来了，因为不是政府建了，就变成私人的了（因为后文提到是自己建，所以选enterprise也不合适）
...
> EF -> BD
> 注意后面的句子`others have performed similar feats.`。**GRE不写废话**
...
> E ->C
> 真看不出这有啥关联。
> 因为study会有bias所以inflate数量(原本太少)。但其实这个数量___(原本就多)
> 关键词是inflating the frequency, 但是这个inflating the frequency是由于bias导致的，并不是指数据真实地变大了，因此inflating the frequency指的是 the frequency本身是很小的，nevertheless表示转折，因此，not + blank = 多，所以选择trivial
...
> AF -> CD
> the wider the appeal, the more (ii)________ the novel(受众越广，越___)应该是对(i)的的解释，假如(i)空选rigorous或exculpatory，则(ii)空没有合适的选项；the wider the appeal, the more undesirable the novel也就是受众越广则越讨厌，这个就和elitist(restricted to an elite)对应。elitist 精英主义的，undesirable不受欢迎的。
> elitist精英主义就是看不起下里巴人，看不起流行通俗。所以越是有广泛吸引力，越是一个负面评价词。/
...
> BDG -> CDH
> 首先第一空出错，以为是他们的argument由于基于anecdote and observations 而大打折扣，忽略了它还没有开始提到*基于* 这些 observation.
> 第三空的link 指代前面的 forest-climate link, 由于前面的困难，导致这个link 和 forest conservation失去关系，所以使用H, I `losing out to` 在某方面失去优势，不符合题意。
...
> BF -> CF
> 这里第一空漏了细节 `we cannot help but share` 说明要为这种行为做一个positive的行为，这里只有C是positive，其它都是negative。所以选择C
...
> AEI -> ADG
> 这里首先把 `subjectivity` 当成 客观了，所以后面两空选项都选反了；其次这题在前面的section中也有提到。
...
> CEI -> BDI
> 首先自己弄不懂B和C那个合适，其次，自己没有琢磨后面的tale直接选了个正面评价，自己仔细看下结构都能发现第二空是个让步，填一个负面词，且与前面的 tale 相关。 
> Lifespan 就像一个轴，时间长短概念转变成一个长度概念，类似于时间轴，所以使用demarcate而不是antedate更合适。
...
> AC -> BE
> 首先自己犯错没有读完题目就着急下选项，看到daunting还有同义词 sensational 直接选了没有继续看后文，犯了大错。。
> 后半句说现在的视觉艺术充斥着达达主义，强调currently，然后如果这个展览不及时进行的话就来不及了，这个风潮就过了，蹭不上热度了。
...
> CF -> AF
> 这里看出一二空是取同，但是第一空在AC之间纠结，看到最后说吸引到几乎不能被夸大的东西上，就错解为能吸引肯定就有夸大它几乎不能夸大的地方，也就是有偏见？？
...
> BEH -> BDH
> 这里第二空选择错误理解了 `as no surprise` 这个意思，以为是惊讶到读者，，倒推出第二空读者没有意识到主编的probity.
...
> CEH -> CFG
> 首先第一空由 `find reason for hope` 说明前面是没有希望的，也就是C。第二个没有注意到数量关系，前面是 `highly` 后面提到 `mix-up` 对应的是 `occasional` 两者不对应，说明第二空不是指这个。第三个看错情感色彩，首先对于 candidate来说 这种 mix-ups应该是不好的，后面的despite接的是一个Positive词汇，也指明这第三空应该是一个negative的词汇，所以就是G。回到第二空，这时可以看出这空应该是一个负面词汇，此时只剩下F可选了。 
> ① 这个道题目题干属于长难句类型题干。通篇只有一句话。拿到长难句最常用的处理方式是先提取句子主干成分。那么这个题干的主干句是“Observers do sometimes find reason for hope”，读者可以先在题干中标出主干。  
> ② 第一二空格所在句子是一个宾语从句修饰前面的Observers，第一空格表示这些观察家对于一个什么样的竞选活动(productions that pass for campaigns)的一种态度。第二空格修饰这个活动的属性。貌似看起来附近没有什么可靠的线索。  
> ③ 继续看第三空格，表示的是偶尔出现的混乱(occasional mix-ups)对游说过程中的候选人来说一种动作。那么正常情况下，在竞选游说过程中(candidates on the trail)出现了混乱对于候选人来说肯定是不利的，所以第三空格填入一个负评价即可。G选项，rattle，表示“使不安”，符合推理。H，表示“支持”;I，表示“使合法”显然不对。  
> ④ 继续往后面读，despite引导让步，说“尽管有着政治策略家们在安排每一件事情的时候都有着军官般战术上的精确(despite the presence of political strategists plotting every event with the tactical precision of military commanders )”，所以这里其实点名了竞选活动(productions that pass for campaigns)的特点，即tactical precision，所以第二空格要体现出这些活动的特点，填入一个类似的概念。F，choreographed，表示“精心设计的”，符合。D，E明显不对。  
> ⑤ 就剩第一空格。第一空表示这些observers对于这些精心设计的活动感到怎么样了于是寄希望于偶尔的混乱(find reason for hope in the occasional mix-ups)，也即，在对A感动什么样的情况下会寄希望于B。显然，第一空格应当填入一个表示“失望”的类似概念。所以只有C，表示“绝望”，符合推理。
