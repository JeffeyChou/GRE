...
#Logic-error #Semantic-error #review_8-24
...
#Logic-error #Incaution-error #review_8-24 
...
#Semantic-error #Incaution-error #review_8-24 
...
#Logic-error #Semantic-error #Supposition-error #review_8-24 
...
#Logic-error #Incaution-error #review_8-26
...
#Semantic-error #Incaution-error #review_8-26
...
# section 14
#Semantic-error #Supposition-error #review_8-26
...
#Semantic-error #Question #review_8-26
...
#Incaution-error #Supposition-error #review_8-26
...
#Logic-error #Semantic-error #review_8-26
...
#Semantic-error #Incaution-error #Supposition-error #review_8-26
...
#Incaution-error #Logic-error #review_8-26
...
#Logic-error #Semantic-error #Incaution-error #review_8-26
...
#Semantic-error #Logic-error #review_8-26
...
> 这里主要是自己没有仔细思考，究竟是支持这个 mainstream 还是 sidestep。后面看到 `herbal remedies` 还没有反应过来这是一种抵制 mainstream 的做法，错选到 CF。只怪在 CN 使用 herb 治疗确实算是一个小 mainsteam 了。
...
> BDH -> BFH
> 从3空入手，even if 资金来源不重要，但是资金来源（iii)了信任。所以填vitiate。credibility 对应skepticism，因为A、C两空的核心意思完全无关所以排除。第一句和第二句中间没有任何转折的词汇，没有but，however，也没有disagree，所以两句话的意思是一致的（对我知道很同意看到后边的 fund 选错）
...
> 我们都知道 although 前后两个句子的关系是对比关系，那么前面有一个“do not”, 后面又一个“fail”,其实就跟做简单的数学题一样，等式的左右都有一个负号，我们就同时约掉。现在前面就剩“think highly of”, 前后句由于存在一个 although，于是取反，后面应该是表示跟“称赞”相反的词汇，那么，DF 正好符合逻辑
...
> BDH -> BDI
> 首先第一空对应problem, 填入负面词，最合适就是 B。第二空和 `unaware of limitations` 对应，填入 B。第三空则是根据第二空的 `belie` 填入和 `relible` 相反的词，这里最好的就是 I。
...
**句子结尾中有个 if __ 属于让步成分，从当转折作用，做题时取 __ 的反义。** (参考 section 81 T7)
...
> 第三空的 link 指代前面的 forest-climate link, 由于前面的困难，导致这个 link 和 forest conservation 失去关系，所以使用 H, I `losing out to` 在某方面失去优势，不符合题意。
